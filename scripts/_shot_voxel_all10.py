#!/usr/bin/env python3
"""Screenshot VOXEL GRID for all TOP-10 + 2x5 contact sheet."""
...
LOGS = ROOT / "logs"
CONTACT = LOGS / "voxel_cube_v2_50k_all10_contact_sheet.png"
EDGE = next(
    p
    for p in [
...
async def shot_one(ws, imo: str) -> dict:
    ...
    (async () => {{
      ...
      if (!tab || tab.classList.contains('is-disabled')) {{
        return {{ ok:false, error:'voxel disabled', ready:v.voxel_cubes }};
      }}
      tab.click();
    ...
    out = LOGS / f"voxel_cube_v2_50k_{imo}.png"
...
def build_contact(vessels):
    ...
    draw.text(
        (pad, 10),
        "VOXEL GRID v2 — TOP-10 · ~50k · 7-color palette · honest envelope",
        fill=(224, 247, 252),
        font=font_t,
    )
    for i, v in enumerate(vessels[:10]):
        ...
        imo = str(v["imo"])
        path = LOGS / f"voxel_cube_v2_50k_{imo}.png"
        if path.exists():
            img = Image.open(path).convert("RGB")
            img.thumbnail((cell_w, cell_h))
            sheet.paste(img, (x + (cell_w - img.width) // 2, y + (cell_h - img.height) // 2))
        draw.text((x, y + cell_h + 4), f"#{v.get('rank')} {v.get('name')} · IMO {imo}", fill=(148, 163, 184), font=font)
    sheet.save(CONTACT)
    return CONTACT
...
async def main() -> int:
    ...
    kill_port(PORT)
    profile = LOGS / "edge_cdp_voxel_all10"
    ...
    try:
        ...
        async with websockets.connect(page["webSocketDebuggerUrl"], max_size=64 << 20) as ws:
            ...
            for v in TOP10_VESSELS:
                ...
                results[imo] = meta
                print(json.dumps({"imo": imo, "ok": meta.get("ok"), "n": meta.get("n"), "mode": meta.get("mode")}, ensure_ascii=False))
        contact = build_contact(TOP10_VESSELS)
        report = {"shots": results, "contact_sheet": str(contact)}
        (LOGS / "voxel_cube_all10_shots.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
        print(json.dumps({"contact": str(contact), "n_ok": sum(1 for r in results.values() if r.get("ok"))}, indent=2))
