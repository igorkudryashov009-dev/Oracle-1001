"""
TOP-200 Analytics — ORACLE-1001
================================
Generates ``output/top200_analytics.html`` — 15 unique SVG/Canvas dashboards
...
OUTPUT = ROOT / "output"
OUT_HTML = OUTPUT / "top200_analytics.html"
...
def _ops_zone(
    ...
    if tech == "VLCC_SUEZ" or any(k in vt for k in ("CRUDE", "VLCC", "НЕФТ", "TANKER")):
        return "OIL_OTHER"
    return "OIL_OTHER"
...
def build_top200_payload(df: pd.DataFrame) -> dict:
    """Select TOP-200 by DWT and build lean records + fleet / provenance totals."""
    records = df.to_dict(orient="records")
    try:
        from pipeline.top200_provenance_fix import (
            fix_top200_provenance,
            summarize_top200_provenance,
        )
        records = fix_top200_provenance(records, top_n=200)
        prov = summarize_top200_provenance(records, top_n=200)
    ...
    return {
        ...
        "fleet_count": fleet_n,
        "top200_dwt": round(top_dwt, 0),
        "top200_share_pct": round(100.0 * top_dwt / fleet_dwt, 2) if fleet_dwt else 0,
        "avg_fill_pct": avg_fill,
        "provenance": prov,
        "vessels": vessels,
        "rest_sample": rest_sample,
        "copyright": "© ORACLE-1001 · ТОП-200 Analytics · NASA / Wet-Glass",
    }
...
<title>ORACLE-1001 · ТОП-200 Analytics · Premium</title>
...
<link rel="stylesheet" href="js/top200_premium.css"/>
</head>
<body>
<nav class="nav">
  <a class="brand" href="osint_layers.html">ORACLE-1001 · ТОП-200</a>
  <div class="nav-links">
    ...
    <a class="pill active" href="top200_analytics.html">Анализ ТОП-200</a>
    <a class="pill" href="top500_analytics.html">Анализ ТОП-500</a>
  </div>
</nav>

<div class="hero">
  <h1>ТОП-200 ПО ДЕДВЕЙТУ · 15 ИНФОГРАФИКОВ</h1>
...
<section class="panel on" id="t1">
  <div class="grid">
    <div class="card wide d13-block" id="c13">
      ...
      <div class="d13-trio" id="d13Trio">
        <div class="d13-pane">
          ...
          <div class="sub">Ближний Восток · Атлантика/США · Арктика · АТР · Прочие</div>
          <div class="chart" id="c13a"></div>
        </div>
        <div class="d13-pane">
          <h4>Д13.2 · Технологические Типы</h4>
          <div class="sub">Q-Flex/Q-Max · Membrane · Moss · FLNG/FSRU · VLCC</div>
    ...
    <div class="card"><h3>Д14 · Scatter GT / Global % DWT</h3><div class="sub">ТОП-200 vs остальной флот · ось X = глобальный %</div><div class="chart" id="c14"></div></div>
    ...
    <div class="card"><h3>Д16 · ТОП портов</h3><div class="sub">Длина бара = вклад порта в глобальный DWT флота</div><div class="chart" id="c16"></div></div>
...
<script>window.__TOP200_PAYLOAD__ = __PAYLOAD__;</script>
<script src="js/top200_engine.js"></script>
...
def write_top200_analytics(df: pd.DataFrame, path: Path | None = None) -> dict:
    """Build TOP-200 payload, copy premium JS/CSS, write analytics HTML."""
    import shutil
    ...
    web = ROOT / "web"
    for name in ("top200_engine.js", "top200_premium.css", "top_validation_suite.js"):
        src = web / name
        if src.exists():
            shutil.copy2(src, js_dir / name)

    payload = build_top200_payload(df)
    ...
    stamp.write_text(
        json.dumps(
            {
                "module": "top200_analytics",
                "copyright": payload.get("copyright"),
                "top200_dwt": payload.get("top200_dwt"),
                "top200_share_pct": payload.get("top200_share_pct"),
                "provenance_pct": payload.get("provenance", {}).get("pct"),
                "vessels": len(payload.get("vessels", [])),
                "assets": ["js/top200_engine.js", "js/top200_premium.css"],
            },
            ensure_ascii=False,
            indent=2,
        ),
...
def main() -> int:
    ...
    payload = write_top200_analytics(df, OUT_HTML)
    print(
        f"Wrote {OUT_HTML} · vessels={len(payload['vessels'])} · "
        f"DWT={payload['top200_dwt']:,.0f} · share={payload['top200_share_pct']:.2f}% · "
