...
- **OOB zero-touch (1.8.0):** `scripts/bootstrap_oob.py` (dirs incl. `data/db` / `.env` vs `config_keys` / intel mocks + `data/cache/mock_fallback_data.json`); `services/ais_worker.py` (G3-safe buffer → `data/cache/ais_live.json`, backoff 2s→60s, no second WS; `--ingest` delegates to `aisstream_connector`); `services/healthcheck.py` + `cache_swr.py` (stale-while-revalidate on news/FIRMS/market); health `oob` plane on `/api/v1/health`; compose aliases `sentinel_api_edge`≡web · `sentinel_ais_worker`≡ais-buffer (`oob-buffer` profile); entrypoint runs bootstrap on container start.
- **Full image bake (1.8.0):** `deploy_sentinel.py --bake-image --force` packs Sync-Tree, seeds 7 intel keys into Node A `.env` (never logged), runs `deploy_korolev_sentinel.sh` build + `--force-recreate` + `output_artifacts` seed; BAKE_OK asserts intel adapters; post-bake smoke requires news/FIRMS/market HTTP 200. Compose interpolates all 7 registry keys into container env.
