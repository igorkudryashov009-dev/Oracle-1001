...
- **OOB zero-touch (1.8.0):** `scripts/bootstrap_oob.py` (dirs / `.env` vs `config_keys` / intel mocks); `services/ais_worker.py` (G3-safe buffer → `data/cache/ais_live.json`, no second WS); `services/healthcheck.py` + `cache_swr.py` (stale-while-revalidate on news/FIRMS/market); health `oob` plane on `/api/v1/health`; compose profile `oob-buffer` for optional buffer plane; entrypoint runs bootstrap on container start.
