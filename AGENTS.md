...
7b. **VF-500 Optimal Allocator (v1.8.0)** — `services/vf_budget_allocator.py`: monthly hard-cap **500** (warn &lt;20); split **300/200** top-DWT / rest; daily **16** (10+6); weekly cap **112**; P1 terrestrial gap / P2 STS·sanctions·ghost hotlist / P3 RR. Daily archive (`archive_snapshot_worker` → `vessel_daily_archive`) covers full gas registry with `source∈{terrestrial_ais,vf_api,none}` only — **never** feeds `fleet_sample_status`. Health exposes `fleet_archive` reporting block.
