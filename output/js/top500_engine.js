/**
 * ORACLE-1001 · TOP-500 Analytics Engine
...
(function () {
  ...
  const PAYLOAD = window.__TOP500_PAYLOAD__;
  if (!PAYLOAD) {
    console.error("TOP-500: PAYLOAD missing");
    return;
  }
  ...
  const TOP500_DWT = PAYLOAD.top500_dwt;
  const TOP500_SHARE = PAYLOAD.top500_share_pct;
  ...
  const ZONE_COLORS = {
    ...
    APAC: "#10b981", LATAM_AFRICA: "#ec4899", OIL_OTHER: "#8aa4bf",
  };
  const TECH_COLORS = {
  ...
  function tipBlock({ title, n, dwt, sliceDwt, extra, risk }) {
    ...
    const topShare = localShare(dwt, TOP500_DWT);
    return `<b>${title}</b><br>` +
      `<span class="gold">Глобальный DWT:</span> <b>${fmtPct(gPct(dwt))}</b> (от 81.3 М т)<br>` +
      `Доля в ТОП-500: <b>${fmtPct(topShare, 1)}</b>` +
  ...
  function safeChart(fn) {
    return function (rows, el) {
      ...
      try {
        ...
        console.error("[TOP-500 chart]", err);
        emptyOverlay(el);
      }
    };
  }
  ...
  class CrossFilterManager {
    ...
    broadcast(rows) {
      ...
      const payload = { imos, ts: Date.now(), source: "top500", sector: this.active(), filters: Object.assign({}, this.filters) };
      try { localStorage.setItem("oracle1001_cross_filter", JSON.stringify(payload)); } catch (_e) {}
      window.__TOP500__ = {
        count: this.all.length, fleetDwt: FLEET_DWT, topDwt: TOP500_DWT, share: TOP500_SHARE,
  ...
  function renderKPI(rows) {
    ...
    document.getElementById("kpiRow").innerHTML = `
      <div class="kpi"><div class="k">Суда в срезе</div><div class="v">${rows.length}</div><div class="s">на ТОП-500</div></div>
      ...
      <div class="kpi"><div class="k">% глобального вклада</div><div class="v">${fmtPct(gPct(dwt))}</div><div class="s">${AXIS_Y}</div></div>
      <div class="kpi"><div class="k">Fill Rate</div><div class="v">${fmtPct(avgFill >= 99.5 ? 100 : avgFill, 0)}</div><div class="s">20 колонок ТЗ</div></div>
  ...
  let d13Mode = (() => {
    try {
      const m = localStorage.getItem("top500_d13_view_mode");
  ...
  const ZONE_SHORT = {
    ...
    APAC: "АТР", LATAM_AFRICA: "Латам/Африка", OIL_OTHER: "Прочие",
  };
  const TECH_SHORT = {
    QFLEX_QMAX: "Q-Flex/Q-Max", MEMBRANE: "Membrane", MOSS: "Moss",
    FLNG_FSRU: "FLNG/FSRU", VLCC_SUEZ: "VLCC/Suezmax/Aframax",
  };
  const CHART_KIND = {
  ...
  // ── D14 Scatter: densе Canvas α=0.75 + micro-cluster grid ──────────────────
  function render14(rows, el) {
    const { w, h } = svgBox(el);
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
    el.innerHTML = `<canvas width="${Math.round(w * dpr)}" height="${Math.round(h * dpr)}" style="width:${w}px;height:${h}px"></canvas><div class="d14-hint">scroll = zoom · micro-cluster</div>`;
    ...
    const pts = [
      ...rows.map((v) => ({ gp: gPct(v.dwt_tons), y: v.gt || 0, top: true, v })),
      ...REST.map((v) => ({ gp: gPct(v.dwt_tons), y: v.gt || 0, top: false, v: null })),
    ];
    ...
    ctx.fillText(AXIS_Y + " → · densе " + rows.length, pad.l, 16);
    ...
    // Micro-cluster when many points / low zoom: bucket into grid cells
    const cell = d14Zoom >= 1.6 ? 6 : (d14Zoom >= 1.15 ? 10 : 14);
    const useCluster = rows.length > 180 || pts.length > 350;
    if (useCluster && d14Zoom < 2.2) {
      const buckets = new Map();
      pts.forEach((p) => {
        const px = X(p.gp), py = Y(p.y);
        const key = `${Math.floor(px / cell)}:${Math.floor(py / cell)}`;
        let b = buckets.get(key);
        if (!b) { b = { n: 0, topN: 0, sx: 0, sy: 0, sample: null }; buckets.set(key, b); }
        b.n += 1; b.sx += px; b.sy += py;
        if (p.top) { b.topN += 1; if (!b.sample) b.sample = p.v; }
      });
      buckets.forEach((b) => {
        const cx = b.sx / b.n, cy = b.sy / b.n;
        const r = Math.min(14, 2.2 + Math.sqrt(b.n) * (b.topN ? 1.4 : 0.7));
        ctx.beginPath();
        ctx.arc(cx, cy, r, 0, Math.PI * 2);
        ctx.globalAlpha = 0.75;
        ctx.fillStyle = b.topN ? "rgba(0,242,254,0.75)" : "rgba(138,164,191,0.28)";
        ctx.fill();
        if (b.n >= 4) {
          ...
          ctx.fillText(String(b.n), cx, cy + 3);
          ctx.textAlign = "start";
        }
      });
      ctx.globalAlpha = 1;
      ...
      pts.forEach((p) => {
        ctx.beginPath();
        ctx.arc(X(p.gp), Y(p.y), p.top ? 3.2 : 1.4, 0, Math.PI * 2);
        ctx.globalAlpha = 0.75;
        ctx.fillStyle = p.top ? "rgba(0,242,254,0.88)" : "rgba(138,164,191,0.28)";
        ctx.fill();
      });
      ctx.globalAlpha = 1;
    }
    ...
    const hitTest = (mx, my) => {
      ...
      rows.forEach((v) => {
        const d = Math.hypot(X(gPct(v.dwt_tons)) - mx, Y(v.gt || 0) - my);
    ...
    c.onwheel = (e) => {
      e.preventDefault();
      d14Zoom = Math.max(1, Math.min(3, d14Zoom + (e.deltaY < 0 ? 0.15 : -0.15)));
      render14(rows, el);
    };
  }
  ...
  function render16(rows, el) {
    ...
    const topDep = Object.entries(dep).sort((a, b) => b[1] - a[1]).slice(0, 20);
    const topDst = Object.entries(dst).sort((a, b) => b[1] - a[1]).slice(0, 20);
    ...
    let g = `<text x="${w / 2}" y="12" text-anchor="middle" class="axis-caption">${AXIS_Y} · ТОП-20 портов среза</text>
  ...
  // ── D18 Treemap Origin → Destination (paginated, risk-colored) ─────────────
  const D18_PAGE_SIZE = 12;
  function render18(rows, el) {
    ...
    const allItems = Object.values(map).sort((a, b) => b.dwt - a.dwt);
    if (!allItems.length) { emptyOverlay(el); return; }
    const pages = Math.max(1, Math.ceil(allItems.length / D18_PAGE_SIZE));
    if (d18Page >= pages) d18Page = 0;
    const items = allItems.slice(d18Page * D18_PAGE_SIZE, d18Page * D18_PAGE_SIZE + D18_PAGE_SIZE);
    const total = items.reduce((s, x) => s + x.dwt, 0) || 1;
    const sliceDwt = dwtSum(rows);
    const chartH = h - 28;
    let x = 0, y = 0, rowH = chartH / 2, g = "", row = 0;
    g += `<text x="${w / 2}" y="12" text-anchor="middle" class="axis-caption">${AXIS_Y} · Origin → Destination · стр. ${d18Page + 1}/${pages}</text>`;
    items.forEach((it) => {
    ...
    el.innerHTML = `<svg viewBox="0 0 ${w} ${chartH}">${g}</svg>
      <div class="pager"><span>${allItems.length} маршрутов · ${(performance.now() - t0).toFixed(1)}ms</span>
      <button type="button" data-dir="-1" ${d18Page <= 0 ? "disabled" : ""}>◀</button>
      <button type="button" data-dir="1" ${d18Page >= pages - 1 ? "disabled" : ""}>▶</button></div>`;
    el.querySelectorAll("rect.seg").forEach((r) => {
      const html = tipBlock({
    ...
    el.querySelectorAll(".pager button").forEach((b) => {
      b.onclick = (e) => {
        e.stopPropagation();
        d18Page = Math.max(0, Math.min(pages - 1, d18Page + Number(b.dataset.dir)));
        render18(rows, el);
      };
    });
  }
  ...
  document.querySelectorAll(".d13-mode").forEach((b) => {
    b.onclick = () => {
      ...
      try { localStorage.setItem("top500_d13_view_mode", d13Mode); } catch (_e) {}
      syncD13ModeUI();
      renderAll();
    };
  });
