"""
TOP-500 Provenance Fixer — ORACLE-1001
======================================
Extends the TOP-100 provenance priority to the DWT TOP-500 slice:

  OSINT (explicit) → AIS (registry_mock) → KNN (imputed) → Synthetic (0%)

Mutates provenance tag fields in-place on the TOP-500 subset only.
"""
...
# Extra promote-to-explicit for TOP-500 audit surface
...
def fix_top500_record(rec: dict[str, Any]) -> dict[str, Any]:
    ...
    # Force zero synthetic on D07 audit columns for TOP-500
...
def fix_top500_provenance(
    ...
    *,
    ...
    for i in ranked_idx:
        fix_top500_record(records[i])
    return records


def summarize_top500_provenance(
    records: list[dict[str, Any]],
    top_n: int = 500,
) -> dict[str, Any]:
